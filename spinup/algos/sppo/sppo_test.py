...
def sppo(args, env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
        steps_per_epoch=8000, epochs=50, gamma=0.997, clip_ratio=0.2,
         train_pi_iters=80, train_v_iters=80, lam=0.95, max_ep_len=200,
    ...
    if args.alpha == 'auto':
        ...
        log_alpha = tf.get_variable('log_alpha', dtype=tf.float32, initializer=tf.log(0.001))
        alpha = tf.clip_by_value(tf.exp(log_alpha), 0.001, 9999990.2)
        ...
        alpha_optimizer = MpiAdamOptimizer(learning_rate=6e-4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='Breakout-ram-v4') # CartPole-v0 Acrobot-v1 Breakout-ram-v4 # 'LunarLanderContinuous-v2' 0.02 #  LunarLander-v2 0.05
    ...
    parser.add_argument('--exp_name', type=str, default='Breakout_sppo_alpha_autov_cpu8_8000_scheme1')#'LL2_cpu4_6000_alphaAuto_et0.35_simgaNew1.0')
    ...
    env = FrameStack(BreakoutWrapper(env,2),2)

    sppo(args, lambda : env, actor_critic=core.mlp_actor_critic,
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l), gamma=args.gamma, max_ep_len=args.max_ep_len,
        seed=args.seed, steps_per_epoch=args.steps, epochs=args.epochs, lam=args.lam,

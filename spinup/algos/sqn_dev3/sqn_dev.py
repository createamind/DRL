...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        ...
        self.pi_log2_buf = np.zeros([size, act_dim[1]], dtype=np.float32)
        self.rews_buf = np.zeros(size, dtype=np.float32)
        self.done_buf = np.zeros(size, dtype=np.float32)
        self.ptr, self.size, self.max_size = 0, 0, size

    def store(self, obs, act, pi_log2, rew, next_obs, done):
        self.obs1_buf[self.ptr] = obs
        self.obs2_buf[self.ptr] = next_obs
        self.acts_buf[self.ptr] = act
        self.pi_log2_buf[self.ptr] = pi_log2
...
def maxsqn(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
    ...
    pi_log2_ph = core.placeholder(act_dim)
    ...
    logp_pi = tf.reduce_sum(tf.exp(pi_log) * pi_log, axis=1)
...
######
    ...
    v_backup = min_q_pi - alpha * pi_log2_ph
    v_backup = tf.reduce_sum(tf.exp(pi_log2_ph)*v_backup, axis=1)
    ...
    for t in range(total_steps):
        ...
        pi_log2 = get_pi_log(o2)
        # r_pi = r + gamma * (1 - d) * (- alpha * logp_pi2)
        # Store experience to replay buffer
        replay_buffer.store(o, a, pi_log2, r, o2, d)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', '-s', type=int, default=9)
    parser.add_argument('--epochs', type=int, default=3000)
    parser.add_argument('--max_ep_len', type=int, default=900)    # make sure: max_ep_len < steps_per_epoch
    parser.add_argument('--alpha', default=0.2, help="alpha can be either 'auto' or float(e.g:0.2).")
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--exp_name', type=str, default='sqn_dev3-buffer1e5-a')

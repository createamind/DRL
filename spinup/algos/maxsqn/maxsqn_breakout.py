...
if __name__ == '__main__':
    ...
    # parser.add_argument('--alpha', default=0.2, help="alpha can be either 'auto' or float(e.g:0.2).")
    parser.add_argument('--lr', type=float, default=1e-3)
    parser.add_argument('--exp_name', type=str, default=f'debug_paper_{parser.parse_args().env}_alpha_{parser.parse_args().alpha}')
    args = parser.parse_args()

    from spinup.utils.run_utils import setup_logger_kwargs
    logger_kwargs = setup_logger_kwargs(args.exp_name, args.seed)

    env_breakout = gym.make('Breakout-ram-v4')
    ...
    env_3 = BreakoutWrapper(gym.make(args.env), 3)
    env_1 = BreakoutWrapper(gym.make(args.env), 1)

...
def sac1_rnn(env_fn, actor_critic=core.mlp_actor_critic, sac1_dynamic_rnn=core.sac1_dynamic_rnn, ac_kwargs=dict(),
             seed=0, Lb=10, Lt=10, hc_dim=128,
    ...
    state_burn_in = tf.stop_gradient(state_burn_in) * data01_ph[:, 0][..., tf.newaxis]  # TODO this line is suspect
    s_outputs, _ = sac1_dynamic_rnn(obs_train, state_burn_in)
    s_ph = s_outputs[:, :-1]
    s2_ph = s_outputs[:, 1:]

    logp_pi, q1, q2, q1_pi, q2_pi = [None, ] * Lt, [None, ] * Lt, [None, ] * Lt, [None, ] * Lt, [None, ] * Lt
    logp_pi_, _, _, q1_pi_, q2_pi_ = [None, ] * Lt, [None, ] * Lt, [None, ] * Lt, [None, ] * Lt, [None, ] * Lt

    for i in range(Lt):
        # Main outputs from computation graph
        with tf.variable_scope('main', reuse=tf.AUTO_REUSE):
            _, _, logp_pi[i], q1[i], q2[i], q1_pi[i], q2_pi[i] = actor_critic(s_ph[:, i, :], a_ph[:, i, :], **ac_kwargs)

        # Target value network
        with tf.variable_scope('target', reuse=tf.AUTO_REUSE):
            _, _, logp_pi_[i], _, _, q1_pi_[i], q2_pi_[i] = actor_critic(s2_ph[:, i, :], a_ph[:, i, :], **ac_kwargs)

    logp_pi, q1, q2, q1_pi, q2_pi = tf.stack(logp_pi, axis=1), tf.stack(q1, axis=1), tf.stack(q2, axis=1), \
                                    tf.stack(q1_pi, axis=1), tf.stack(q2_pi, axis=1)
    ...
    # TODO:I think here we should use logp_pi_ instead of logp_pi
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='sac1_rnn_BipedalWalkerHardcore-v2_debug_auto')
    ...
    sac1_rnn(lambda x: env_train if x == 'train' else env_test, actor_critic=core.mlp_actor_critic,

...
env = gym.make(GAME)
S_DIM, A_DIM = env.observation_space.shape[0], env.action_space.shape[0]  # state and action dimension
...
class PPO(object):
    def __init__(self):
        ...
        # actor
        pi, pi_params = self._build_a_net('pi',
                                          trainable=True)  # pi is a policy with a normal distribution N(mu,sigma), pi_params are the tf.GraphKeys.GLOBAL_VARIABLES
        ...
        ratio = tf.reduce_sum(pi.prob(self.tfa), axis=1, keep_dims=True) / (tf.reduce_sum(oldpi.prob(self.tfa),
                                                                                          axis=1,
                                                                                          keep_dims=True) + 1e-5)
        ...
        self.entropy = -ALPHA * tf.reduce_sum(oldpi.log_prob(self.sample_op), axis=1, keep_dims=True)
        self.logp_a = -ALPHA * tf.reduce_sum(oldpi.log_prob(self.tfa), axis=1, keep_dims=True)

        self.adv_logp = self.advantage + self.logp_a  # TODO
    ...
    def update(self):
        ...
        while not COORD.should_stop():
            if GLOBAL_EP < EP_MAX:
                ...
                #                print(oldpi_pro)
                [self.sess.run(self.a_train_op, {self.tfs: s, self.tfa: a, self.tfadv: adv}) for _ in
                 range(UPDATE_STEP)]
...
def main(args):
    ...
    if not args.Test:
        ...
        vtest = GLOBAL_PPO.sess.run(GLOBAL_PPO.v, {
            GLOBAL_PPO.tfs: [[np.cos(th * factor1), np.sin(th * factor1), 0] for th in range(1000)]})
        # print(vtest)
